<x-layout :sectionName="__('Create')" :pageName="__('User')">
    ...
    <div class="flex justify-center">
        ...
        <div class="w-1/2 flex md:justify-between flow-root rounded-lg border border-gray-100 py-3 shadow-sm bg-white justify-center items-center text-center">
            <form action="{{ route('user.store') }}" method="POST" class="w-full p-4">
                @csrf
                @if ($errors->any())
                ...
                <dl class="-my-3 divide-y divide-gray-100 text-sm dark:divide-gray-700">
                    ...
                    <div class="grid grid-cols-1 gap-1 p-3 even:bg-gray-50 sm:grid-cols-3 sm:gap-1 even:dark:bg-gray-800 items-center">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name') }}" class="text-gray-700 sm:col-span-2 dark:text-gray-200 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Name">
                    </div>

                    <!-- Email -->
                    <div class="grid grid-cols-1 gap-1 p-3 even:bg-gray-50 sm:grid-cols-3 sm:gap-1 even:dark:bg-gray-800 items-center">
                        <label for="email" class="font-medium text-gray-900 dark:text-white">Email</label>
                        <input type="email" id="email" name="email" value="{{ old('email') }}" class="text-gray-700 sm:col-span-2 dark:text-gray-200 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Email">
                    </div>
                    ...
                    <!-- Submit Button -->
                    <div class="flex justify-center">
                        <button type="submit" title="Create" class="w-16 h-16 text-indigo-600 bg-indigo-50 rounded-full duration-150 hover:bg-indigo-100 active:bg-indigo-200 flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
